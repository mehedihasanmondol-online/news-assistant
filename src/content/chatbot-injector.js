...
(function () {
  ...
  function findSendButton(inputEl) {
    if (currentBot === 'chatgpt') {
      return (
        ...
        inputEl?.parentElement?.parentElement?.querySelector('button:last-child')
      );
    }
  ...
  async function submitPrompt(inputEl) {
    // Wait slightly for reactive UI state to enable the submit button
    ...
    if (sendBtn && !sendBtn.disabled && sendBtn.getAttribute('aria-disabled') !== 'true') {
      sendBtn.click();
      console.log('[News Assistant] Clicked send button');
      return true;
    }
